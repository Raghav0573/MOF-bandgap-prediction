...
1. **inference.ipynb**: To run inference on your own data. Load the pretrained model weights (model.pt) and the three scaler.pkl files to predict the band gap energy levels in metal organic frameworks (MOFs) using an Auto-encoder architecture. The input data consists of .cif structure files and a .csv file with the required columns. 
